...
func (am *AuthMiddleware) RecordEarlyError(r *http.Request, userID, email, team, person, errorType, errorMessage string) {
	...
	// Determinar model_id desde el contexto si está disponible
	modelID := "unknown"
	if userCtx, err := GetUserFromContext(r.Context()); err == nil {
		if userCtx.DefaultInferenceProfile != "" {
